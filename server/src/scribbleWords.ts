...
export const scribbleWords: string[] = [
  // Animals
 ...
 */
export function generateWordOptions(): string[] {
  const shuffled = [...scribbleWords].sort(() => Math.random() - 0.5);
 ...
 */
export function generateWordHint(word: string, revealedIndices: number[]): string {
  return word
    .split('')
    .map((char, i) => {
      if (char === ' ') return '  ';
