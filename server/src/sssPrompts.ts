 ...
 */
export function generatePromptSet(playerCount: number): { realPrompt: string; variants: string[] } {
  if (playerCount < 3) {
 ...
 */
export function getUnusedTemplateIndex(usedIndices: Set<number>): number {
 ...
 * @returns Object with realPrompt and array of scrambled variants
 */
export function generatePromptSetFromTemplate(templateIndex: number, playerCount: number): { realPrompt: string; variants: string[] } {
...
export const TEMPLATE_COUNT = promptTemplates.length;
